...
const repeatedDataSample = Array.from({ length: 10 }, () => dataSample).flat();
const uniqueKeysDataSample = repeatedDataSample.map((item, index) => ({ ...item, key: index }));
...
export default function AcountScreen() {
    ...
    const _onPressSample = () => {
        ...
        const image = uniqueKeysDataSample[0].image;
        addFavorite(image);
        console.log(image);
    };
